...
class BasePoissonAlgorithm(BaseBayesianAlgorithm):
...
class EpsilonGreedyAlgorithm(BasePoissonAlgorithm):
...
class BayesUCBAlgorithm(BasePoissonAlgorithm):
    ...
    def single_step(self, t):
        if t < self.K:
            ...
            quantiles = gamma.ppf(quantile, self.alphas, scale=1 / self.betas)
...
class ThompsonSamplingAlgorithm(BasePoissonAlgorithm):
    def __init__(self, bandit_env):
        super().__init__(bandit_env)
    ...
    def single_step(self, t):
        ...
        if t < self.K:
            ...
            lambda_hats = gamma.rvs(self.alphas, scale=1 / self.betas, size=self.K)
...
class VarianceIDSAlgorithm(BasePoissonAlgorithm):
